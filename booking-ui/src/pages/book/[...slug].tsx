...
class Search extends React.Component<Props, State> {
  ...
  loadItems = () => {
    ...
    const directBookingLocation = {
      locationId: "",
      spaceId: ""
    };

    if(this.props.router.query.slug && this.props.router.query.slug.length === 2) {
      directBookingLocation.locationId = this.props.router.query.slug[0];
      directBookingLocation.spaceId = this.props.router.query.slug[1];
    }
    ...
    Promise.all(promises).then(() => {
      ...
      if (this.state.locationId === "" && this.locations.length > 0) {
        ...
        if(directBookingLocation.locationId) {
          if(this.locations.findIndex(location => location.id === directBookingLocation.locationId) != -1) {
            defaultLocationId = directBookingLocation.locationId;
          }
        }
        this.setState({ locationId: defaultLocationId }, () => {
          this.loadMap(this.state.locationId).then(() => {
            if(directBookingLocation.spaceId != "") {
              const space = this.data.find(space => space.locationId === directBookingLocation.locationId
                && space.id === directBookingLocation.spaceId
              );
              ...
              if(!space) {
                this.setState({
                  loading: false,
                  showError: true,
                  errorText: this.props.t("errorPickArea")
                });
              } else {
                this.onSpaceSelect(space);
              }
            }
